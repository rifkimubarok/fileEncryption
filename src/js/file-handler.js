...
export default class FileHandler {
    ...
    if (window.Worker) {
      ...
      hideWorker.onmessage = (e) => {
        ...
        if (e.data.progress === 4) {
          ...
          setTimeout(() => {
            progressText.html('Enkripsi Berhasil!')
            ...
            const resultOut = $('#resultOut')
            resultOut.html('File Enkripsi:')

            // Print file output.
            const fileName = "haitayo"
            // const fileType = this.hideImage.type.slice(6)
            const filesOut = $('#filesOut')
            this.printFiles([new window.File([resultBlob], fileName)], filesOut)
            ...
            download.attr('download','encryptedFile.rifki_egi')
          }, 600)
      ...
      revealWorker.onmessage = (e) => {
        ...
        if (e.data.progress === 4) {
          ...
          setTimeout(() => {
            ...
            download.attr('href', resultURL)
            download.attr('download','resultDecrypted.zip');
          }, 600)
        } else if (e.data.error) {
          // If there was an error, print error.
          this.calculated = false
  ...
  hideSubmit (e) {
    e.preventDefault()
    if (this.hideFiles && validateHidePassword()) {
      this.hide()
    }
  }
